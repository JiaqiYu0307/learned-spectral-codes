...
const materials=[
  ...
  {key:'gold',title:'Gold conductor',detail:'Measured metal · D65',metric:'Mean ΔE₂₀₀₀: 0.65 codec / 8.98 RGB',description:'The spectral Fresnel response preserves the hue and intensity of metallic reflection.'},
  ...
  {key:'sss-hand',title:'Heterogeneous volume',detail:'Per-voxel spectral hand',metric:'Mean ΔE₂₀₀₀: 0.93 codec / 4.00 RGB',description:'Spatially varying skin features, including freckles, knuckles and veins, remain distinguishable.'}
];
const engines=[
...
function comparisonCard(item,type){const engine=type==='engine';const startLeft=engine?'codec':'latent';const leftChoices=engine?[['codec',labels.codec],['ours',labels.ours]]:[];const rightChoices=[['rgb',labels.rgb],['gt',labels.gt]];return `<article class="comparison-card" data-key="${item.key}"><div class="card-top"><div><span class="eyebrow">${item.detail}</span><h3>${item.title}</h3></div><span class="card-index">${engine?'RENDERER':'MATERIAL'}</span></div><p>${item.description}</p>${engine?`<div class="card-controls"><span>Show</span><div class="segmented" role="group" aria-label="${item.title} method">${leftChoices.map(([value,label],i)=>`<button type="button" data-side="left" data-value="${value}" aria-pressed="${i===0}">${label}</button>`).join('')}</div></div>`:''}<div class="card-controls"><span>Compare with</span><div class="segmented" role="group" aria-label="${item.title} comparison">${rightChoices.map(([value,label],i)=>`<button type="button" data-side="right" data-value="${value}" aria-pressed="${i===0}">${label}</button>`).join('')}</div></div><div class="comparison mini-comparison" style="--split:50%"><img class="compare-base" src="assets/${item.key}-rgb.webp" alt="${item.title}: RGB baseline" loading="lazy"><img class="overlay compare-overlay" src="assets/${item.key}-${startLeft}.webp" alt="${item.title}: ${labels[startLeft]}" loading="lazy"><div class="divider"><span>↔</span></div><span class="image-label left compare-left-label">${labels[startLeft]}</span><span class="image-label right compare-right-label">RGB baseline</span><input type="range" min="0" max="100" value="50" aria-label="Move the divider in the ${item.title} comparison"></div>${item.metric?`<p class="card-metric">${item.metric}</p>`:''}</article>`}
...
document.querySelectorAll('[data-comparison]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-comparison]').forEach(other=>{const active=other===button;other.classList.toggle('selected',active);other.setAttribute('aria-pressed',active)});const value=button.dataset.comparison;document.querySelector('#base-image').src=`assets/${value}.webp`;document.querySelector('#base-image').alt=`Final paper teaser: ${labels[value]}`;document.querySelector('#right-label').textContent=labels[value]}));
document.querySelectorAll('.comparison-card .segmented button').forEach(button=>button.addEventListener('click',()=>{const card=button.closest('.comparison-card');const side=button.dataset.side;card.querySelectorAll(`[data-side="${side}"]`).forEach(other=>other.setAttribute('aria-pressed',other===button));const value=button.dataset.value;const image=card.querySelector(side==='left'?'.compare-overlay':'.compare-base');image.src=`assets/${card.dataset.key}-${value}.webp`;image.alt=`${card.querySelector('h3').textContent}: ${labels[value]}`;card.querySelector(side==='left'?'.compare-left-label':'.compare-right-label').textContent=labels[value]}));
